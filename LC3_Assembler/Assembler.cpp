...
int s_find(string str, char c, int time)//This function return the position of the time'th character 'c' in the string str
{
	int ret = -1;
...
string trans_BR(string sen)
{
	...
	{
		int d3 = max(max(s_find(br_part, 'R', 1) + 1, s_find(br_part, 'N', 1) + 1), max(s_find(br_part, 'Z', 1) + 1, s_find(br_part, 'P', 1) + 1)) + s_find(sen, 'B', 1);
		string last = sen.substr(d3, sen.length() - d3);
		string label = get_first_word(last);
		int address = 3000;
...
string trans_TRAP(string sen)
{
	...
	else//written in "TRAP vector8" formula
	{
		...
		trap_vector8 = dec_to_2com(atoi_hex(sen.substr(d, sen.length() - d).c_str()), 8);
	}
...
//a combining translating function, call the trans_ function according to "type"
//notice that "sen" is the lc-3 line that has been pretreat, type is usually its first word 
string translate(string sen, string type)
{
	if (type == "ADD")
		...
		type == "BRNZ" || type == "BRNP" || type == "BRZP" || type == "BRNZP")
	{
		return trans_BR(sen);
	}
	else if (type == "TRAP" || type == "GETC" || type == "OUT"
		|| type == "PUTS" || type == "IN" || type == "PUTSP" || type == "HALT")
...
//Capitalization -> Comments Deleting ->Label Deleting and Symbol_Adrress table construction 
void Pretreat(string& txt)
{
	...
	while (txt[first] == '\0')
		first++;
	int i = first, j = 0;
	//For the first traversal , turn all English letters in txt (except the string in "") into uppercase to avoid trouble
	//disadvantage: Lables can't be distinguished by upper or lower case of it's character
	int flag = 1;
	while (txt[i] != '\0')
	{
	...
	//the second traversal, delete comments, delete labels and construct the symbol_address table.
	i = first;
	string line = "";
	line.resize(300);
	current_address = -1;
	...
	while (txt[i] != '\0')
	{
		...
		if (fw == ".ORIG")
		{
			start_address = atoi_hex(line.substr(s_find(line, 'G', 1) + 1, line.length() - s_find(line, 'G', 1)));
		...
		else if (fw != "ADD" && fw != "AND" && fw != "NOT" && fw != "LD" && fw != "LDR" && fw != "LDI" && fw != "ST" &&
			fw != "STR" && fw != "STI" && fw != "TRAP" && fw != "LEA" && fw != "RTI" && fw != "JMP" && fw != "JSR" &&
			fw != "RET" && fw != "JSRR" && fw != ".FILL" && fw != ".STRINGZ" && fw != ".BLKW" && fw != "BR" && fw != "BRN" &&
			fw != "BRZ" && fw != "BRP" && fw != "BRNZ" && fw != "BRNP" && fw != "BRZP" && fw != "BRNZP" && fw != "GETC" &&
			fw != "PUTS" && fw != "PUTSP" && fw != "IN" && fw != "OUT" && fw != "HALT" && fw != ".END" && fw[0] != '\0')
		{
...
//after pretreat, every line in txt can be direcly translated, then do the assemble
